    ...
    quote do
      ...
      if (unquote(required.call_forwarding_catchall)) do
        ...
        def call_forwarding_catchall(call, context, _from, %__MODULE__{} = this) do
          if context do
            Logger.warn("[#{context.token}] #{__MODULE__} Unhandled Call #{inspect {call, __MODULE__.ref(this)}}")
          else
            Logger.warn("[NO_TOKEN] #{__MODULE__} Unhandled Call #{inspect {call, __MODULE__.ref(this)}}")
          end
  ...
  defmacro __before_compile__(_env) do
    quote do
      ...
      def call_forwarding(call, context, _from, %__MODULE__{} = this) do
        if context do
          Logger.warn("[#{context.token}] Unhandled Call #{inspect {call, __MODULE__.ref(this)}}")
        else
          Logger.warn("[NO_TOKEN] Unhandled Call #{inspect {call, __MODULE__.ref(this)}}")
        end
        {:reply, :unsupported_call, this}
      end
      #-----------------------------------------------------------------------------
