...
defmodule Noizu.SimplePool.V2.Server.WorkerManagement.DefaultImplementation do
  ...
  #---------------------------------------------------------------------------
  def status(server, context), do: server.i_call(:status, context)
  ...
  #---------------------------------------------------------------------------
  # Internal Routing - i_call_handler
  #---------------------------------------------------------------------------
  def i_call_handler(module, {:lock!, options}, context, _from, %State{} = this), do: lock!(this, context, options)
  def i_call_handler(module, {:release!, options}, context, _from, %State{} = this), do: release!(this, context, options)
  def i_call_handler(module, {:health_check!, health_check_options}, context, _from, %State{} = this), do: get_health_check(this, context, health_check_options)
  def i_call_handler(module, {:load, options}, context, _from, %State{} = this), do: load_workers(this, context, options)
  def i_call_handler(module, call, context, _from, %State{} = this) do
  ...
  #---------------------------------------------------------------------------
  # Internal Routing - i_cast_handler
  #---------------------------------------------------------------------------
  def i_cast_handler(module, {:server_kill!, options}, context, %State{} = this) do
    server_kill!(this, context, options)
  end

  def i_cast_handler(module, call, context, %State{} = this) do
  ...
  #---------------------------------------------------------------------------
  def i_info_handler(module, call, context, %State{} = this) do
