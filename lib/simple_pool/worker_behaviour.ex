...
defmodule Noizu.SimplePool.WorkerBehaviour do
  ...
  def default_init({_mod, server, base, _worker_state_entity, _inactivity_check, _lazy_load}, {:migrate, ref, initial_state, context}) do
  ...
  def default_init({_mod, server, base, _worker_state_entity, _inactivity_check, _lazy_load}, {ref, context}) do
    br = :os.system_time(:millisecond)
    server.worker_lookup_handler().register!(ref, context)
    task = server.worker_lookup_handler().set_node!(ref, context)
    r = Task.yield(task, 75)
    ...
    quote do
      ...
      if (unquote(required.delayed_init)) do
        def delayed_init(state, context) do
          default_delayed_init({__MODULE__, @server, @base, @worker_state_entity, @inactivity_check, @lazy_load}, state, context)
        end
      end
